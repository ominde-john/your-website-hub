...
const MessageBubble = ({ message, isMine }) => {
  ...
  return (
    <motion.div 
      ...
      className={`flex gap-2 md:gap-4 group mb-4 md:mb-6 ${isMine ? "flex-row-reverse" : "flex-row"}`}
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      {/* 1. STYLIZED AVATAR */}
      <div className="relative flex-shrink-0 self-end mb-1">
        <div className={`w-7 h-7 md:w-9 md:h-9 rounded-xl md:rounded-2xl overflow-hidden border-2 transition-transform duration-500 hover:rotate-6 ${
        ...
        {!isMine && (
          <span className="absolute -top-1 -right-1 w-2 h-2 md:w-3 md:h-3 bg-emerald-500 border-2 border-white rounded-full"></span>
        )}
      </div>
      ...
      <div className={`flex flex-col ${isMine ? "items-end" : "items-start"} max-w-[85%] md:max-w-[75%]`}>
        ...
        <div className="flex items-center gap-1 md:gap-2 mb-1 px-1">
          ...
          <span className="text-[9px] md:text-[10px] text-slate-400 font-medium">{message.timestamp}</span>
        </div>

        {/* 3. THE BUBBLE */}
        <div className="relative">
          <div
            className={`px-3 py-2 md:px-5 md:py-3 rounded-2xl md:rounded-[2rem] shadow-sm transition-all duration-300 ${
              isMine
            ...
            } ${isCodeSnippet ? "w-full min-w-[200px] md:min-w-[300px]" : ""}`}
          >
            {isCodeSnippet ? (
              <div className="rounded-xl overflow-hidden">
                <div className={`flex items-center justify-between px-2 md:px-3 py-1.5 md:py-2 ${isMine ? 'bg-indigo-700/50' : 'bg-slate-50 border-b border-slate-100'}`}>
                  <div className="flex items-center gap-2">
                    <Terminal size={12} className={isMine ? "text-indigo-200" : "text-indigo-500"} />
                    <span className={`text-[9px] md:text-[10px] font-bold uppercase tracking-widest ${isMine ? "text-indigo-100" : "text-slate-500"}`}>Snippet</span>
                  </div>
                  <button className="p-1 hover:bg-white/10 rounded-md transition-colors">
                    <Copy size={10} className={isMine ? "text-indigo-200" : "text-slate-400"} />
                  </button>
                </div>
                <pre className={`p-2 md:p-4 text-[10px] md:text-xs font-mono overflow-x-auto ${isMine ? 'bg-indigo-900/30' : 'bg-slate-900 text-slate-300'}`}>
                  <code>{cleanText}</code>
                </pre>
              </div>
            ) : (
          ...
          {/* 4. CONTEXTUAL ACTION TOOLBAR (VISIBLE ON HOVER - hidden on mobile for touch friendliness) */}
          <AnimatePresence>
            {isHovered && (
              <motion.div 
                ...
                exit={{ opacity: 0, y: 5 }}
                className={`absolute ${isMine ? 'right-0' : 'left-0'} hidden md:flex items-center bg-white shadow-2xl shadow-indigo-200 border border-slate-100 p-1 rounded-2xl z-10`}
              >
        ...
        {isMine && (
          <div className="flex items-center gap-1 mt-1 md:mt-1.5 px-1">
            <span className="text-[9px] md:text-[10px] text-slate-400 font-bold uppercase tracking-tighter">Delivered</span>
            {message.read ? (
              <CheckCheck className="w-2.5 h-2.5 md:w-3 md:h-3 text-indigo-500" />
            ) : (
              <Check className="w-2.5 h-2.5 md:w-3 md:h-3 text-slate-300" />
            )}
          </div>
        )}
      </div>
