...
const SidebarItem = ({ item, isActive, onClick, expanded, isMobile = false }) => {
  ...
  if (isMobile) {
    return (
      <button onClick={onClick} className={`relative flex flex-col items-center justify-center p-2 flex-1 min-w-0 transition-all duration-300 group ${isActive ? "text-techgold" : "text-slate-500 hover:text-techgold"}`}>
        ...
        <span className={`text-xs mt-1 truncate max-w-full font-medium ${isActive ? "text-techgold" : "text-slate-500"}`}>{item.label}</span>
  ...
  return (
    <button onClick={onClick} className={`relative flex items-center transition-all duration-300 rounded-2xl group overflow-hidden ${expanded ? "w-full px-4 py-3.5 justify-start" : "w-14 h-14 justify-center mx-auto"} ${isActive ? "bg-gradient-to-r from-techgold via-amber-500 to-amber-600 text-white shadow-xl shadow-techgold/30" : "text-slate-600 hover:bg-white/60 hover:shadow-lg hover:text-techgold"}`}>
      ...
      <div className="relative z-10 flex items-center w-full">
        ...
        {expanded && (
          <>
            <span className={`ml-3 font-semibold text-sm ${isActive ? "text-white" : "text-slate-700 group-hover:text-techgold"}`}>{item.label}</span>
...
const Sidebar: React.FC<SidebarProps> = ({ activeNav, onNavChange, darkMode, setDarkMode, userProfile }) => {
        ...
        {mobileMenuOpen && (
          <>
            ...
            <div className="fixed top-0 left-0 h-full w-80 bg-gradient-to-br shadow-2xl z-50 border-r overflow-y-auto transition-colors duration-300" style={{
              background: darkMode ? 'linear-gradient(to bottom right, rgb(15 23 42), rgb(30 41 59))' : 'linear-gradient(to bottom right, white, rgba(254, 243, 199, 0.3), rgba(253, 230, 138, 0.5))',
              ...
              <div className="p-3">
                {navItems.map(item => (
                  <button key={item.id} onClick={() => { onNavChange(item.id); setMobileMenuOpen(false); }} className={`flex items-center w-full p-3 rounded-2xl mb-2 transition-all ${activeNav === item.id ? "bg-gradient-to-r from-techgold to-amber-600 text-white shadow-xl" : "text-slate-700 hover:bg-white/60"}`}>
  ...
  return (
    <>
      <nav className={`hidden md:flex h-screen bg-gradient-to-br from-slate-50 via-amber-50/30 to-yellow-50/50 border-r border-white/20 flex-col transition-all duration-500 shadow-2xl relative ${expanded ? "w-80" : "w-20"}`}>
        <div className="absolute inset-0 bg-gradient-to-br from-techgold/5 to-amber-500/5 opacity-50"></div>
        <div className="absolute top-0 right-0 w-64 h-64 bg-techgold/10 rounded-full blur-3xl animate-pulse"></div>
        ...
        <div className="relative p-4 border-t border-white/20 space-y-3 bg-white/30 backdrop-blur-xl z-10">
          <div className={`flex gap-2 ${expanded ? "justify-between" : "justify-center"}`}>
            <button onClick={() => onNavChange("notifications")} className="p-2.5 hover:bg-white/60 rounded-xl relative transition-all group">
              <Bell className="w-5 h-5 text-slate-600 group-hover:text-techgold" />
